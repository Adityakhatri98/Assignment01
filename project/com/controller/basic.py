...
UPLOAD_FOLDER = 'project/static/uploadFile/'
...
def search():
    ...
    reader = csv.DictReader(open("project/static/uploadFile/people.csv"))
...
def editDetails():
   f = open("project/static/uploadFile/people.csv", "r")
   csvr=csv.reader(f)
   found=0
   m1=[]
   name = request.form['name']
   ...
   if(found==0):
       ...
       f=open("project/static/uploadFile/people.csv", "w",newline='')
       csvr=csv.writer(f)
       csvr.writerows(m1)
       f.close()
